...
def set_model_cpx_3(case_params):
    '''
    ...
    (n_interval, delta_t, n_timewindow_set, n_timewindow_effect, large_m,
     n_scenario, scenario_weight, sn_mva, MW_KW) = (
        case_params['n_interval'],
        case_params['delta_t'], case_params['n_interval_window_set'],
        case_params['n_interval_window_effect'], case_params['large_m'],
        case_params['n_scenario'], case_params['scenario_weight'],
        case_params['sn_mva'], case_params['MW_KW'])

    # Parameters for ds
    (ppnet, idx_ref_bus, idx_pv_bus, idx_pq_bus, idx_mg_bus, nb, nl, nd,
     branch_r, branch_x, connection_ds_f, connection_ds_t, idx_beta_ij,
     idx_beta_ji, p_load_profile, q_load_profile, load_inter_cost,
     load_inter_cost_augment, idx_load, connection_load, pij_u, qij_u, sij_u,
     pij_l, qij_l,v0, vm_u, vm_l, slmax, pd_u, qd_u, load_qp_ratio) = (
        case_params['ppnet'], case_params['idx_ref_bus'],
        case_params['idx_pv_bus'], case_params['idx_pq_bus'],
        case_params['idx_mg_bus'], case_params['nb'],
        case_params['nl'], case_params['nd'], case_params['branch_r'],
        case_params['branch_x'], case_params['connection_ds_f'],
        case_params['connection_ds_t'], case_params['idx_beta_ij'],
        case_params['idx_beta_ji'], case_params['pload_profile'],
        case_params['qload_profile'], case_params['load_inter_cost'],
        case_params['load_inter_cost_augment'], case_params['idx_load'],
        case_params['connection_load'], case_params['pij_u'],
        case_params['qij_u'], case_params['sij_u'], case_params['pij_l'],
        case_params['qij_l'], case_params['v0'], case_params['vm_u'],
        case_params['vm_l'], case_params['slmax'], case_params['pd_u'],
        case_params['qd_u'], case_params['load_qp_ratio'])

    # Parameters for mg
    (n_mg, connection_mg, localload_p, localload_q, pmg_u, qmg_u, pmg_l, qmg_l,
     energy_capacity_ratio, energy_u, energy_l, cost_mg) = (
        case_params['n_mg'], case_params['connection_mg'],
        case_params['localload_p'], case_params['localload_q'],
        case_params['pmg_u'], case_params['qmg_u'], case_params['pmg_l'],
        case_params['qmg_l'], case_params['energy_capacity_ratio'],
        case_params['energy_u'], case_params['energy_l'],
        case_params['cost_mg'])

    # Parameters for ts
    (n_station, connection_station, n_tsn_node, idx_tsn_node, n_tsn_arc,
     tsn_arc_table, cut_set_tsn, idx_tsn_arc_parking, connection_tsn_f,
     connection_tsn_t, is_location_station) = (
        case_params['n_station'], case_params['connection_station'],
        case_params['n_tsn_node'], case_params['idx_tsn_node'],
        case_params['n_tsn_arc'], case_params['tsn_arc_table'],
        case_params['cut_set_tsn'], case_params['idx_tsn_arc_parking'],
        case_params['connection_tsn_f'], case_params['connection_tsn_t'],
        case_params['is_location_station'])
    # Parameters for ev
    (n_ev, ev_energy_capacity, soc_init, soc_max, soc_min, ev_energy_u,
     ev_energy_l, ev_energy_init, ev_dch_efficiency, ev_ch_efficiency,
     ev_dch_u, ev_ch_u, loss_on_road, cost_ev_transit, cost_ev_power,
     ev_position_init) = (case_params['n_ev'],
        case_params['ev_energy_capacity'], case_params['soc_init'],
        case_params['soc_max'], case_params['soc_min'],
        case_params['ev_energy_u'], case_params['ev_energy_l'],
        case_params['ev_energy_init'], case_params['ev_dch_efficiency'],
        case_params['ev_ch_efficiency'], case_params['ev_dch_u'],
        case_params['ev_ch_u'], case_params['loss_on_road'],
        case_params['cost_ev_transit'], case_params['cost_ev_power'],
    ...
    # TESS model
    # charging power from mg to ev at time span t, 3D-array,
    # (n_ev, n_mg, n_interval)
    var_pev2st_ch_x = array(model_x.variables.add(
        names=['ch_ev{0}_st{1}_t{2}'.format(i_ev, j_station, k_t)
               for i_ev in range(n_ev)
               for j_station in range(n_station)
               for k_t in range(n_interval)])
        ).reshape(n_ev, n_station, n_interval)

    # discharging power from ev to mg at time span t, 3D-array,
    # (n_ev, n_mg, n_interval)
    var_pev2st_dch_x = array(model_x.variables.add(
        names=['dch_ev{0}_st{1}_t{2}'.format(i_ev, j_station, k_interval)
               for i_ev in range(n_ev)
               for j_station in range(n_station)
    ...
    var_ev_soc_x = array(model_x.variables.add(
        lb=soc_min.ravel(), ub=soc_max.ravel(),
    ...
    var_sign_ch_x = array(model_x.variables.add(
        types=['B'] * (n_ev * n_interval),
    ...
    var_sign_dch_x = array(model_x.variables.add(
        types=['B'] * (n_ev * n_interval),
    ...
    # arc status for ev at time span t, 3-D array, (n_ev, n_tsn_arc)
    # modify it to fit into new tsn model
    var_ev_arc_x = array(model_x.variables.add(
        types=['B'] * n_ev * n_tsn_arc,
        names=['ev{0}_arc{1}'.format(i_ev, j_arc)
               for i_ev in range(n_ev)
               for j_arc in range(n_tsn_arc)])
        ).reshape(n_ev, n_tsn_arc)

    # Transit status for ev at time span t, 2-D array, (n_ev, n_interval)
    var_sign_onroad_x = array(model_x.variables.add(
        types=['B'] * (n_ev * n_interval),
    ...
    var_pmg_x = array(model_x.variables.add(
        lb=pmg_l.ravel(), ub=(pmg_u - localload_p).ravel(),
    ...
    var_qmg_x = array(model_x.variables.add(
        lb=(qmg_l - localload_q).ravel(), ub=(qmg_u - localload_q).ravel(),
    ...
    var_emg_x = array(model_x.variables.add(
        lb=energy_l.ravel(), ub=energy_u.ravel(),
    ...
    var_pij_x = array(model_x.variables.add(
    ...
    var_qij_x = array(model_x.variables.add(
    ...
    var_vm_x = array(model_x.variables.add(
    ...
    # aggregated active power generation at DS level, 2-D array,
    # (n_mg, n_interval)
    var_pg_x = array(model_x.variables.add(
        names=['pg{0}_t{1}'.format(i_mg, j_interval)
               for i_mg in range(n_mg)
               for j_interval in range(n_interval)])
        ).reshape(n_mg, n_interval)
    # aggregated reactive power generation at DS level, 2-D array,
    # (n_mg, n_interval)
    # lb = full((n_mg, n_interval), fill_value=-inf).ravel()
    var_qg_x = array(model_x.variables.add(
        lb=-cpx.infinity * ones((n_mg, n_interval)).ravel(),
        names=['qg{0}_t{1}'.format(i_mg, j_interval)
               for i_mg in range(n_mg)
               for j_interval in range(n_interval)])
        ).reshape(n_mg, n_interval)
    ...
    var_gama_load_x = array(model_x.variables.add(
        types=[model_x.variables.type.binary] * (nd * n_interval),
    ...
    var_alpha_branch_x = array(model_x.variables.add(
        types=['B'] * nl,
    ...
    var_betaij_x = array(model_x.variables.add(
        types=['B'] * nl,
        names=['betaij_{0}'.format(i_l)
               for i_l in range(nl)])
        ).reshape(nl)
    var_betaji_x = array(model_x.variables.add(
        types=['B'] * nl,
    ...
    case_params = add_dict_items(
        case_params, var_pev2mg_ch_x=var_pev2st_ch_x,
        var_pev2mg_dch_x=var_pev2st_dch_x, var_ev_soc_x=var_ev_soc_x,
        var_sign_ch_x=var_sign_ch_x, var_sign_dch_x=var_sign_dch_x,
        var_ev_arc_x=var_ev_arc_x, var_sign_onroad_x=var_sign_onroad_x,
        var_pmg_x=var_pmg_x, var_qmg_x=var_qmg_x, var_emg_x=var_emg_x,
        var_pij_x=var_pij_x, var_qij_x=var_qij_x, var_vm_x=var_vm_x,
        var_pg_x=var_pg_x, var_qg_x=var_qg_x, var_gama_load_x=var_gama_load_x,
        var_alpha_branch_x=var_alpha_branch_x, var_betaij_x=var_betaij_x,
        var_betaji_x=var_betaji_x, var_scenario=var_scenario)

    # --------------------------------------------------------------------------
    # First stage model
    # Add Objective function

    # a = zip(var_sign_onroad_x.ravel().tolist(),
    # [cost_ev_transit] * var_sign_onroad_x.size)
    # In comparison with a = zip(var_sign_onroad_x.tolist(),
    # [cost_ev_transit] * var_sign_onroad_x.size)
    # transportation cost
    model_x.objective.set_linear(
        zip(var_sign_onroad_x.ravel().tolist(),
            [cost_ev_transit] * var_sign_onroad_x.size))
    # charging cost
    model_x.objective.set_linear(
        zip(var_pev2st_ch_x.ravel().tolist(),
            [cost_ev_power * delta_t * sn_mva * MW_KW] * var_pev2st_ch_x.size))
    # discharging cost
    model_x.objective.set_linear(
        zip(var_pev2st_dch_x.ravel().tolist(),
            [cost_ev_power * delta_t * sn_mva * MW_KW] * var_pev2st_dch_x.size))
    # generation cost
    model_x.objective.set_linear(
        zip(var_pmg_x.ravel().tolist(),
            [cost_mg * delta_t * sn_mva * MW_KW] * var_pmg_x.size))
    # load interruption cost
    model_x.objective.set_linear(
        zip(var_gama_load_x.ravel().tolist(),
            (-load_inter_cost_augment * pd_u).ravel()
            * delta_t * sn_mva * MW_KW))
    ...
    # Each EV only in one status in each time interval
    aeq_onestatus = lil_matrix((n_ev * n_interval, n_all_vars_x))
    beq_onestatus = ones((n_ev * n_interval))

    for i_ev in range(n_ev):
        for j_interval in range(n_interval):
            aeq_onestatus[i_ev * n_interval + j_interval,
                          var_ev_arc_x[i_ev, cut_set_tsn[:, j_interval]]] = 1
    ...
    aeq_transitflow = lil_matrix((n_ev * n_tsn_node, n_all_vars_x))
    beq_transitflow = zeros(n_ev * n_tsn_node)

    # for i_ev in range(n_ev):
    #     for j_interval in range(n_interval - 1):
    #         aeq_transitflow[ix_(i_ev * n_interval * n_station + j_interval
    # * n_station + arange(n_station),
    #                             var_ev_arc_x[i_ev, :, j_interval])] =
    # station_connection_t
    #         aeq_transitflow[ix_(i_ev * n_interval * n_station + j_interval
    # * n_station + arange(n_station),
    #                             var_ev_arc_x[i_ev, :,
    #                             j_interval + 1])] = -station_connection_f
    #  station_connection_f needs
    #         # to be converted into array if aeq_ is array.
    #     # For t = 0 and intial status, but note it is put at the last.
    #     aeq_transitflow[ix_(((i_ev + 1) * n_interval - 1) * n_station
    # + arange(n_station),
    #                         var_ev_arc_x[i_ev, :, 0])] = station_connection_f
    #     beq_transitflow[((i_ev + 1) * n_interval - 1) * n_station
    # + arange(n_station)] = ev_position_init[:, i_ev]  # ev_position_init can
    # only be ndarray other than csr_matrix matrix

    for i_ev in range(n_ev):
        # For the tsn nodes that reside in the first time point
        j_tsn_node_range = arange(idx_tsn_node[0, 1])
        aeq_transitflow[ix_(i_ev*n_tsn_node+j_tsn_node_range,
            var_ev_arc_x[i_ev, :])] = connection_tsn_f[j_tsn_node_range, :]
        beq_transitflow[i_ev*n_tsn_node+
                        j_tsn_node_range] = ev_position_init[:, i_ev]

        # For the tsn nodes that reside from the 2nd time point to the
        # last-to-second time point
        j_tsn_node_range = arange(idx_tsn_node[0, 1], idx_tsn_node[0, -1])
        aeq_transitflow[ix_(i_ev*n_tsn_node+j_tsn_node_range, var_ev_arc_x[i_ev,
            :])] = connection_tsn_t[j_tsn_node_range, :] \
                   - connection_tsn_f[j_tsn_node_range, :]
    ...
    model_x_senses += ['E'] * n_ev * n_tsn_node

    # charging/discharging with respect to position
    aineq_pchposition = lil_matrix((n_ev*n_interval*n_station, n_all_vars_x))
    bineq_pchposition = zeros(n_ev*n_interval*n_station)
    aineq_pdchposition = lil_matrix((n_ev*n_interval*n_station, n_all_vars_x))
    bineq_pdchposition = zeros(n_ev*n_interval*n_station)

    for i_ev in range(n_ev):
        for j_interval in range(n_interval):
            # pch <= ***Pch,max
            aineq_pchposition[ix_(i_ev*n_interval*n_station
                                  + j_interval*n_station + arange(n_station),
                        var_pev2st_ch_x[i_ev, :, j_interval])] = eye(n_station)
            # idx_tsn_arc_parking includes non-station location, so use
            # is_location_station to mask.
            aineq_pchposition[ix_(i_ev*n_interval*n_station
                                  + j_interval*n_station + arange(n_station),
                    var_ev_arc_x[i_ev, idx_tsn_arc_parking[is_location_station,
                                   j_interval]])] = -ev_ch_u * eye(n_station)

            # Pdch <= ***Pdch,max
            aineq_pdchposition[ix_(i_ev*n_interval*n_station
                                   + j_interval*n_station + arange(n_station),
                    var_pev2st_dch_x[i_ev, :, j_interval])] = eye(n_station)
            aineq_pdchposition[ix_(i_ev*n_interval*n_station
                                   + j_interval * n_station + arange(n_station),
                    var_ev_arc_x[i_ev, idx_tsn_arc_parking[is_location_station,
                                    j_interval]])] = -ev_dch_u * eye(n_station)

    model_x_matrix_a = vstack([model_x_matrix_a, aineq_pchposition,
                               aineq_pdchposition])
    model_x_rhs += (bineq_pchposition.tolist() + bineq_pdchposition.tolist())
    model_x_senses += ['L'] * 2 * n_ev * n_interval * n_station
    ...
    for i_ev in range(n_ev):
        for j_interval in range(n_interval):
            aineq_pchstatus[i_ev*n_interval + j_interval,
                            var_pev2st_ch_x[i_ev, :, j_interval]] = 1
            aineq_pchstatus[i_ev*n_interval + j_interval,
                            var_sign_ch_x[i_ev, j_interval]] = -ev_ch_u

            aineq_pdchstatus[i_ev*n_interval + j_interval,
                             var_pev2st_dch_x[i_ev, :, j_interval]] = 1
            aineq_pdchstatus[i_ev*n_interval + j_interval,
                             var_sign_dch_x[i_ev, j_interval]] = -ev_dch_u

    model_x_matrix_a = vstack([model_x_matrix_a, aineq_pchstatus,
                               aineq_pdchstatus])
    ...
    for i_ev in range(n_ev):
        for j_interval in range(n_interval):
            aineq_chdchstatus[i_ev*n_interval + j_interval,
                              var_sign_ch_x[i_ev, j_interval]] = 1
            aineq_chdchstatus[i_ev*n_interval + j_interval,
                              var_sign_dch_x[i_ev, j_interval]] = 1
            aineq_chdchstatus[i_ev*n_interval + j_interval,
                var_ev_arc_x[i_ev,
                    idx_tsn_arc_parking[is_location_station, j_interval]]] = -1
    ...
    beq_signonroad = ones(n_ev*n_interval)

    for i_ev in range(n_ev):
        for j_interval in range(n_interval):
            aeq_signonroad[i_ev*n_interval + j_interval,
                           var_sign_onroad_x[i_ev, j_interval]] = 1
            aeq_signonroad[i_ev*n_interval + j_interval,
                var_ev_arc_x[i_ev, idx_tsn_arc_parking[:, j_interval]]] = 1
    ...
    for i_ev in range(n_ev):
        for j_interval in range(n_interval - 1):
            aeq_soc[i_ev*n_interval + j_interval,
                var_ev_soc_x[i_ev, j_interval + 1]] = ev_energy_capacity[i_ev]
            aeq_soc[i_ev*n_interval + j_interval,
                    var_ev_soc_x[i_ev, j_interval]] = -ev_energy_capacity[i_ev]

            aeq_soc[i_ev*n_interval + j_interval,
                    var_pev2st_ch_x[i_ev, :,
                        j_interval + 1]] = -delta_t * ev_ch_efficiency
            aeq_soc[i_ev*n_interval + j_interval,
                    var_pev2st_dch_x[i_ev, :,
                    j_interval + 1]] = delta_t / ev_dch_efficiency

        aeq_soc[i_ev*n_interval + j_interval,
                var_ev_soc_x[i_ev, 0]] = ev_energy_capacity[i_ev]
        aeq_soc[i_ev*n_interval + j_interval,
                var_pev2st_ch_x[i_ev, :, 0]] = -delta_t * ev_ch_efficiency
        aeq_soc[i_ev*n_interval + j_interval,
                var_pev2st_dch_x[i_ev, :, 0]] = delta_t / ev_dch_efficiency
    ...
    # staion and mg need to be maped and linked
    ...
    for i_mg in range(n_mg):
        for j_interval in range(n_interval):
            # pch
            aeq_generationp[i_mg * n_interval + j_interval,
                            var_pev2st_ch_x[:, i_mg, j_interval]] = 1
            # pdch
            aeq_generationp[i_mg * n_interval + j_interval,
                            var_pev2st_dch_x[:, i_mg, j_interval]] = -1
            # pmg
    ...
    aeq_mgenergy = lil_matrix((n_mg * n_interval, n_all_vars_x))
    ...
    aeq_dsoneparent = lil_matrix((nb - n_mg, n_all_vars_x))
    beq_dsoneparent = ones(nb - n_mg)

    aeq_dsoneparent[:, var_betaij_x] = connection_ds_f[idx_pq_bus, :].toarray()
    aeq_dsoneparent[:, var_betaji_x] = connection_ds_t[idx_pq_bus, :].toarray()
    ...
    n_index_betaij = idx_beta_ij.shape[0]
    n_index_betaji = idx_beta_ji.shape[0]
    ...
    aeq_dsnoparent[:n_index_betaij,
        var_betaij_x[idx_beta_ij]] = eye(n_index_betaij)
    aeq_dsnoparent[n_index_betaij:,
        var_betaji_x[idx_beta_ji]] = eye(n_index_betaji)
    ...
    # aeq_dskclpcoe = vstack(((connection_t-connection_f).toarray(), zeros((nb, nl)), zeros((nb, nb)), connection_generator.toarray(), zeros((nb, n_mg)) ), axis=1 )
    # aeq_dskclqcoe = vstack((zeros((nb, nl)), (connection_t-connection_f).toarray(), zeros((nb, nb)), zeros((nb, n_mg)), connection_generator.toarray() ), axis=1 )
    ...
    for j_interval in range(n_interval):
        ...
        aeq_dskclp[ix_(j_interval * nb + arange(nb), var_pij_x[:, j_interval])] = (
                    connection_ds_t - connection_ds_f).toarray()
        aeq_dskclp[ix_(j_interval * nb + arange(nb), var_pg_x[:, j_interval])] = connection_mg.toarray()
        ...
        aeq_dskclq[ix_(j_interval * nb + arange(nb), var_qij_x[:, j_interval])] = (
                    connection_ds_t - connection_ds_f).toarray()
        aeq_dskclq[ix_(j_interval * nb + arange(nb), var_qg_x[:, j_interval])] = connection_mg.toarray()
    ...
    for j_interval in range(n_interval):
        ...
        aineq_dskvl_u[ix_(j_interval * nl + arange(nl), var_vm_x[:, j_interval])] = (
                    connection_ds_t.T - connection_ds_f.T).toarray()
        ...
        aineq_dskvl_l[ix_(j_interval * nl + arange(nl), var_vm_x[:, j_interval])] = (
                    connection_ds_f.T - connection_ds_t.T).toarray()
...
def set_model_grb_1(case_params):
    ...
    (ppc, nb, nl, n_mg, n_mg, nd, n_arc, n_ev, n_interval, n_scenario, n_station, cost_ev_transit, cost_ev_power, delta_t,
     ...
     branch_x, v0, load_qp_ratio, soc_init, load_inter_cost_augment, n_timewindow_c) = (case_params['ppc'], case_params['nb'],
            case_params['nl'], case_params['n_mg'], case_params['n_mg'], case_params['nd'], case_params['n_arc'],
    ...
    # aggregated active power generation at DS level, 2-D array, (n_mg, n_interval)
    var_pg_x = array([model_x.addVar(name='pg{0}_t{1}'.format(i_mg, j_interval))
                      for i_mg in range(n_mg)
                      for j_interval in range(n_interval)],
                     dtype='object').reshape(n_mg, n_interval)

    # aggregated reactive power generation at DS level, 2-D array, (n_mg, n_interval)
    # lb = full((n_mg, n_interval), fill_value=-inf).ravel()
    var_qg_x = array([model_x.addVar(lb=-grb.GRB.INFINITY,
                                     name='qg{0}_t{1}'.format(i_mg, j_interval))
                      for i_mg in range(n_mg)
                      for j_interval in range(n_interval)],
                     dtype='object').reshape(n_mg, n_interval)
