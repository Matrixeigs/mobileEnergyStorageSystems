...
from archive.get_evposition import get_evposition
from archive.get_evpositionpower import get_evpositionpower
from archive.get_mgrev_e import get_mgrev_e
from archive.combine_casefile import combine_ppc
...
def cpx_test(casedata=None):
    ...
    # update data structure that can be self-adaptive to expanding network
    ...
    for j_interval in range(n_interval):
        ...
        if j_interval:
            ...
            # aaa = csr_matrix((ones(n_ev), (ev_position[:, n_interval_window_effect], range(n_ev))),
